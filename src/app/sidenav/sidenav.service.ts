import { ModuleModel } from './../core/model/module/module.model';
...
export class SidenavService {
  ...
  private module:any;
  private moduleModelList:ModuleModel[];
  ...
  constructor(private signUpService:SignUpService,
    ...
    this.signUpService.findAllUsers2().subscribe(response=>{
      console.log("response="+response.json());
      console.log(response.json().id);
    });*/

    this._http.get(this._config.apiEndpoint+"api/modules",this.options())
              .subscribe(response=>{
                this.moduleModelList=response.json();
                for(let moduleModel of this.moduleModelList){
                  let moduleName=this.addItem(moduleModel.name, moduleModel.moduleIcon, null, moduleModel.id);
                  for(let pageModel of moduleModel.pageModuleBeanList){
                    this.addSubItem(moduleName, pageModel.name, pageModel.url, moduleModel.id);
                  }
                }
              })

    /*const dashboard = this.addItem('Home page', 'home', '/home', 1);
    const materials = this.addItem('UI Element', 'bubble_chart', null, 3);
    this.addSubItem(materials, 'Paging', '/materials/pagination', 3);
